...
UBYTE canplayermove(UINT8 newplayerx, UINT8 newplayery){ // RIGHT / LEFT movements
    UINT16 tileindexTL;
...
UBYTE canplayermove_down(UINT8 newplayerx, UINT8 newplayery){ // DOWN ONLY
    UINT16 tileindexTL = 0;
    UINT8 indexTLy, indexTLx, result;
...
void transform_piece(UINT8 newplayerx, UINT8 newplayery){
    ...
    set_bkg_tiles(indexTLx,indexTLy-1,1,1,bloctile); // making 4 of these otherwise doesn't works well
...
void main(){
    ...
    while (1){   
        ...
        if(!keydown){
            switch(keys){
                ...
                case J_UP :
                    ...
                    break;
                case J_DOWN :
                    if(canplayermove_down(bloc.x,bloc.y+8)){ // - 8 prevent bug
                        bloc.y += 8;
                        sound_blocmove();
                    }
                    break;
        ...
        if (frame_skip == 0) {
            ...
            if(canplayermove_down(bloc.x,bloc.y+8)){ // bloc falls from 8 units
                    bloc.y += 8;
            }
